...
COPY --from=builder /workspace/src/main/resources/truststore.p12 /app/truststore.p12

# 컨테이너 실행 시 실행될 명령어 (prod 프로파일 활성화)
ENTRYPOINT ["java", "-jar", "-Dspring.profiles.active=deploy", "-Djavax.net.ssl.trustStoreType=PKCS12", "-Djavax.net.ssl.trustStore=/app/truststore.p12","-Djavax.net.ssl.trustStorePassword=${KEYSTORE_PW}", "app.jar"]
